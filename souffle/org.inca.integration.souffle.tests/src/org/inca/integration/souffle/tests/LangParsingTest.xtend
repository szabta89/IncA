 ...
 */
package org.inca.integration.souffle.tests
...
class LangParsingTest {
	...
	@Inject
	ParseHelper<Analysis> parseHelper
	...
	def void test1() {
		val model = parseHelper.parse('''
			.decl r1(?x:symbol,?y:symbol)
			.decl r2(?x:symbol,?y:symbol,?z:symbol)
			.decl r3(?x:symbol,?y:symbol,?z:symbol)
			r1(?a, ?b) :-
				r2(?a,?b,?c),
				r3(?a,?b,?c).
		''')
		...
		val expansion = model.contents.last as RuleDefinition
		Assert.assertEquals(3, expansion.variables.size)
		val r1 = expansion.head.head
		val r2 = expansion.body.head as RuleApplication
		Assert.assertSame((r1.arguments.head as VariableReference).variable,
			(r2.arguments.head as VariableReference).variable)
		val r3 = expansion.body.last as RuleApplication
		Assert.assertSame((r1.arguments.head as VariableReference).variable,
			(r3.arguments.head as VariableReference).variable)
		Assert.assertSame((r2.arguments.last as VariableReference).variable,
			(r3.arguments.last as VariableReference).variable)
	}
	...
	def void test2s() {
		val result = parseHelper.parse('''
			...
			.decl Reachable(?method:Method) output
